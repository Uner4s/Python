...
def Matriz_DiagonalCero(Matriz_B,orden, lista):
	...
	for contador_2 in range(orden):
		for contador_3 in range(orden):
			Matriz_B[contador_2][contador_3] = float(lista[contador_1])
...
def Matriz_resultadosMatriz(Matriz_C,orden, lista):
	...
	for contador_3 in range(orden):
		Matriz_C[contador_3] = float(Matriz_C[contador_3])
...
def Matriz_Diagonal(Matriz_A,orden, lista):
	...
	while contador_3 != orden :
		Matriz_A[contador_3][contador_1] = float(lista[contador_2])
		contador_1 = contador_1 + 1
...
def metodo_Jacobi(Matriz_A,Matriz_B,Matriz_C,Matriz_Resultados,orden): # Funcion que realiza el metodo de jacobi.
...
def metodo_Seidel(Matriz_A,Matriz_B,Matriz_C,Matriz_Resultados,orden): # Funcion que realiza el metodo de Seidel
...
def funcion_principal_Metodos(orden, lista): # Funcion principal que se encarga de llamar las funciones de jacobi y seidel.
	...
	Matriz_Resultados2 = [0 for col in range(1) for row in range(orden*2)]

	for Contador_Principal in range(orden*2):
		Matriz_Resultados[Contador_Principal] = float(Matriz_Resultados[Contador_Principal])
	Matriz_respaldo = Matriz_Resultados

	Matriz_A = Matriz_Diagonal(Matriz_A, orden, lista) # llamada a la funcion que crea la matriz D, de la Diagonal
	Matriz_B = Matriz_DiagonalCero(Matriz_B, orden, lista) # llamada a la funcion que crea la matriz R, contiene todos los datos y ceros en la diagonal.
	Matriz_C = Matriz_resultadosMatriz(Matriz_C, orden, lista) # llamada a la funcion que crea la matriz B, contiene los resultados de el sistema de ecuaciones.
	os.system('clear')

	for Contador_Principal in range(orden):   # esta funcion se encarga de sacar la condicion de que   | aii |   es mayor que la suma de los demas elementos.
	...
	if condicion == orden: # Si se cumple la condicion, entonces se pueden llamar las funciones de jacobi  y seidel.

		salir2 = 0
		while salir2 != 3:
			continuar = input("\n | Presione ENTER para continuar con el prograama |") # Sistema de pause para continuar con el programa.
			os.system('clear')
			...
			for a in range(orden*2):
				Matriz_Resultados[a] = 0
			print(" ")
			print("****************************************************")
...
def Programa_Matrices_Algebra():
	...
	print("| Version 2.0 |")
	print(" ")
	orden = int(input("| Ingrese el orden de la matriz | = ")) # Se pide el orden de la matriz del archivo por temas de seguridad.
	Archivo= open("Matrices.txt", "r",encoding = 'utf-8') # Se abre el archivo para sacar los datos. 
	lines = Archivo.read() # Todos los datos se pasan a la variable lines.
	lista = lines.split() # Se pasa a  una lista todos los elementos de lines para tener un acceso mas facil a los datos.
	Archivo.close() # Se cierra el archivo ya que no lo usaremos mas.

	if (orden)*(orden)+orden == len(lista): # Si coinciden el orden ingresado con la cantidad de datos dentro del archvo.
		print("|----------------------------------------------------|")
		...
		print(" ")
		print(" ")
		funcion_principal_Metodos(orden, lista) # Se envian el orden de la matriz y la lista con los elementos del archivo.
	else:
		os.system('clear')
		print("|-------------------------------------------------------------------------------------|")
